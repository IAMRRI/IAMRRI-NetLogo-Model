...
globals [
  ...
  net-phase-1                    ; networks in idea generation phase
  net-phase-2                    ; networks in product development phase
  net-pahse-3                    ; networks in market diffusion phase
...
to initialise-AM-techs
  create-AM-techs nAM-tech [
    ...
    set initial-capital  100              ;initial-capital ipotizzato 100 (togliere initial capital come global?)
    ...
    set ethical-thinking random-float 1
    set RRI-value  (list open-access public-engagement ethical-thinking)
...
to make-kene-AM-tech
  ...
  set capabilities n-of cap-capacity (range 1 5)
  ...
  foreach capabilities [i ->
    if i = 1 [ set abilities lput one-of (range 1 9) abilities]
    if i = 2 [ set abilities lput 9 abilities]
    if i = 3 [ set abilities lput 10 abilities]
    if i = 4 [ set abilities lput one-of (range 11 16) abilities]
...
to make-kene-Suppliers
  ...
  ; fill the ability vector with abilities. These are integres and assigned
  foreach capabilities [i ->
    if i = 1 [ set abilities lput one-of [1 2 7 8] abilities]
    if i = 2 [ set abilities lput one-of [9 16] abilities]
    if i = 3 [ set abilities lput one-of [17 18] abilities]
    if i = 4 [ set abilities lput one-of [14 19] abilities]
    ; fill the expertise vector with expertise. These are integers randomly chosen between 1 and 10
    set expertises lput (random 10 + 1) expertises]
end
...
to make-kene-Customers
  ...
  set capabilities n-of cap-capacity [1 4 6]
  set capabilities shuffle capabilities
  ; fill the ability vector with abilities. These are integres and assigned
  foreach capabilities [i ->
    if i = 1 [ set abilities lput one-of [24 25] abilities]
...
to make-kene-OEMs
  ...
  ; fill the capability vector with capabilities.  These are integers
  ; between 1 and 3, such that no number is repeated
  set capabilities n-of cap-capacity [1 4 5]
  set capabilities shuffle capabilities
  ; fill the ability vector with abilities. These are integres and assigned
  foreach capabilities [i ->
    if i = 1 [ set abilities lput 22 abilities]
    if i = 4 [ set abilities lput one-of [11 12 13 19 20 21] abilities]
...
to initialise-Research-insts
  create-Research-insts nResearch-inst [
    ...
    set industry random 3 + 1             ;research field
    set ih []
    set max-veiled 1
    set start-project? false
    set partners no-turtles
...
;firm Customer procedure 6 capabilities; 10 abilities
to make-kene-Research-insts
  ...
  ; fill the capability vector with capabilities.  These are integers
  ; between 1 and 3, such that no number is repeated
  set capabilities n-of cap-capacity [1 2 6 7 8 9]
  set capabilities shuffle capabilities
  ; fill the ability vector with abilities. These are integres and assigned
  foreach capabilities [i ->
    if i = 1 [ set abilities lput 5 abilities]
    if i = 2 [ set abilities lput one-of [9 28] abilities]
    if i = 6 [ set abilities lput 27 abilities]
    if i = 7 [ set abilities lput one-of [29 8 2] abilities]
    if i = 8 [ set abilities lput one-of [30 31] abilities]
...
;firm procedure
to make-advert
  ;set advert capabilities                    ; modify /length ih in procedure compatible,  length capabilities
  set advert map [ i -> item i capabilities ] ih
end
...
;the potential partner with capabilities similar to mine and with a high RRI-value will have greater attractiveness;
to-report compatible? [possible-partner]
  ...
  set attractiveness ((length intersection advert [capabilities] of possible-partner / length ih) * (1 - RRI-attractiveness)
...
to make-RRI
  ...
  set RRI-net (list open-access public-engagement ethical-thinking)
end
...
to update-RRI
   ask net-partners[
    ...
    foreach difference [ i ->
      ...
      if 2 = (position i difference) and ethical-thinking <= 1 [
        if (i < 0.3) and (i > 0) [set ethical-thinking (ethical-thinking + 0.2 / project)]
        if (i > 0.3) and (i < 0.7) [set ethical-thinking (ethical-thinking + 0.14 / project)]
        if (i > 0.7) [set ethical-thinking (ethical-thinking + 0.07 / project)]]
    ]
  ]
end
...
to GATE-NEXT-PHASE-2
  ...
  make-quality
  next-phase-2?]
  set net-phase-2 count networks with [phase = 2]
  set %survived-1 (net-phase-2 / net-phase-1) * 100
...
to less-partner?
  ...
  if count net-partners < min-partners [dissolve-network ]
...
to GATE-NEXT-PHASE-3
  ...
  make-quality
  next-phase-3?]
  let net-phase-3 count networks with [phase = 3]
  if any? networks with [phase = 2][set %survived-2 (net-phase-3 / net-phase-2) * 100]
end

to next-phase-3?
  ifelse ethical-thinking < regulator and quality < standard-organization
  [dissolve-network]
  [set phase 3
  make-start-up]
end
...
to layout-turtles
  ...
  if layout = " UNINA"[
    ...
    ask Suppliers [ setxy 5 / 8 * max-pxcor  3 / 8 * max-pycor
      fd 10 ]
    ask Customers [ setxy -3 / 8 * max-pxcor -1 / 8 * max-pycor
      fd 10 ]
    ask Research-insts [ setxy 3 / 8 * max-pxcor -1 / 8 * max-pycor
      fd 10 ]
...
510
Mean partnerships
sum [count partners]of turtles / count turtles with [not member? self networks]
17
1
11
